...
try:
    ...
    #gdb = r"C:\Users\marlon.ruiz\Documents\Herramientas 2022\Resultados Codificacion\Mitu_P6_50K_V2_PruebaCU.gdb"
    #Codigo_municipio = "97001"
    #B = "P1" #B=parte_trabajada
    #CSV = r"C:\Users\marlon.ruiz\Documents\Herramientas 2022\Resultados Codificacion\codigos\Cod_Unico_NM.csv"
    gdb = arcpy.GetParameterAsText(0)
    Codigo_municipio = arcpy.GetParameterAsText(1)
    B = arcpy.GetParameterAsText(2)
    CSV = arcpy.GetParameterAsText(3)
    arcpy.AddMessage('************************************************************************')
    arcpy.AddMessage((type(CSV)))
    if CSV == '':
        arcpy.AddWarning("No agrego ningun archivo, se creará el diccionario a partir de la siguiente tabla:")
        CSV_STR = pandas.DataFrame(dic)
        arcpy.AddMessage(CSV_STR.to_string())
    else:
        try:
            arcpy.AddMessage("Se agrego el archivo "+CSV.split("\\")[-1]+", se creará el diccionario a partir de la siguiente tabla:")
            CSV_STR = pandas.read_csv(CSV,sep=";",encoding='cp1252')
            arcpy.AddMessage(CSV_STR.to_string())
        except:
            arcpy.AddMessage("Verifique que el delimitador sea punto y coma y que tenga los campos necesarios COD, IDEN ")
    #Variable global para codificación
    M = Codigo_municipio

    #Lógica de negocio
    fcs = listFcsInGDB(gdb)
    fcs.remove(r'NombresGeograficos\NGeogr')
    arcpy.AddWarning("No se códifica el nombre geográfico puesto que este se da a partir de las relaciones del modelo")
    for fc in fcs:
        if int(arcpy.management.GetCount(fc).getOutput(0)):
            ...
            #workspace1 = gdb +os.sep + fc
            #print("Trabajando en "+parte)
            #arcpy.AddMessage("Trabajando en "+(gdb +os.sep + fc).split('\\')[-1])
