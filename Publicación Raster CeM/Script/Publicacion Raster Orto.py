...
try:
    ...
    mosaic = file.split('\\')[-1].split('.')[0].split('_')[0][0:4].lower()+file.split('\\')[-1].split('.')[0].split('_')[1]
    ...
    arcpy.AddMessage('3. Construir Footprint')
    #5. Piramidales 
    ...
    arcpy.AddMessage('4. Construir Piramidales')
    #Agregando el Mosaic al canvas de ArcGIS
    map = aprx.listMaps("Map")[0]
    map.addDataFromPath(os.path.join(gdb,mosaic))
    arcpy.AddMessage('Agregando el Mosaic al canvas de ArcGIS Pro')
    #6. Definir overviews
    ov = mosaic+'OV'
    ...
    arcpy.AddMessage('5. Definir overviews')
    ...
    arcpy.AddMessage('6. Construir overviews ')
